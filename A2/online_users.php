...
.online-users-list li:hover .kick-btn{
opacity:0.7;
}

.online-users-list li .kick-btn:hover{
opacity:1;
}
</style>
</head>
...
<?= $online_count ?> user<?= $online_count !== 1 ? 's' : '' ?> online
</p>
</div>
...
title="<?= htmlspecialchars(ucfirst(str_replace('_',' ',$user['role']))) ?>">
<?= ROLE_ICONS[$user['role']] ?? '?' ?>
</span>

<a href="view_profile.php?user=<?= urlencode($user['username']) ?>"
target="_parent"
style="color: <?= htmlspecialchars($user['chat_color']) ?>; text-decoration:none;">
...
elseif (ROLES[$current_user_role] > ROLES[$user['role']]){
$can_kick=true;
}
...
<form method="POST"
action="kick_user.php"
style="display:inline;"
onsubmit="return askKickReason(this);">
...
form.querySelector(".kick-reason").value = reason;
